...
#define time_duration 30000

const char *phone = "0612230586";
...
void callback(String topic,byte* payload,unsigned int length1){ 
  ...
  if(topic == String("/")+String(phone)+String("/house1/control/spray")){
    if(msgg == "ON_SPRAY"){
      data = String("/")+String(phone)+String("/house1/state/spray");
      data.toCharArray(pub_topic,data.length()+1);
       digitalWrite(SPRAY,HIGH); 
       client.publish(pub_topic,"ON");  
       Serial.println("/buu/iot/diy/state/spray,ON");    
    }else if(msgg == "OFF_SPRAY"){
      data = String("/")+String(phone)+String("/house1/state/spray");
      data.toCharArray(pub_topic,data.length()+1);
       digitalWrite(SPRAY,LOW); 
       client.publish(pub_topic,"OFF");   
       Serial.println("/buu/iot/diy/state/spray,OFF");
    }
  }else if(topic==String("/")+String(phone)+String("/house1/control/fan")){
    if(msgg=="ON_FAN"){
      data = String("/")+String(phone)+String("/house1/state/fan");
      data.toCharArray(pub_topic,data.length()+1);
      digitalWrite(FAN,HIGH);
      client.publish(pub_topic,"ON");
      Serial.println("/buu/iot/diy/state/fan,ON");
    }else if(msgg="OFF_FAN"){
      data = String("/")+String(phone)+String("/house1/state/fan");
      data.toCharArray(pub_topic,data.length()+1);
      digitalWrite(FAN,LOW);
      client.publish(pub_topic,"OFF");
      Serial.println("/buu/iot/diy/state/fan,OFF");
    }
  }else if(topic==String("/")+String(phone)+String("/house1/control/valve1")){
    if(msgg=="ON_VALVE1"){
      data = String("/")+String(phone)+String("/house1/state/valve1");
      data.toCharArray(pub_topic,data.length()+1);
  ...
  }else if(topic==String("/")+String(phone)+String("/house1/control/valve2")){
    if(msgg=="ON_VALVE2"){
      data = String("/")+String(phone)+String("/house1/state/valve2");
      data.toCharArray(pub_topic,data.length()+1);
...
void reconnect(){
   ...
   while (!client.connected()) {
     ...
     if (client.connect(alias, mqttuser, mqttpass )) {      
       ...
       data = String("/")+String(phone)+String("/house1/control/#");
...
void loop(){
    ...
    if (millis()-t_dht >= time_duration) {
        ...
        if (isnan(humid) || isnan(temp)) { //ตรวจสอบค่าที่อ่านได้ว่าเป็น NaN หรือไม่
          data = String("/")+String(phone)+String("/house1/state/error");
          data.toCharArray(pub_topic,data.length()+1);
...
//          client.publish("/buu/iot/diy/state/dht/error","Failed to read from DHT!!!");
          ...
          data = String("/")+String(phone)+String("/house1/state/dht");
          data.toCharArray(pub_topic,data.length()+1);
...
//              client.publish("/buu/iot/diy/state/dht",str); //ส่งข้อมูล ความชื้น และ อุณหภูมิ ไปยัง topic ชื่อ /dht
          data = String("/")+String(phone)+String("/house1/state/temp");
          data.toCharArray(pub_topic,data.length()+1);
          client.publish(pub_topic,str);
//          client.publish("/buu/iot/diy/state/temp",str);
          ...
          data = String(humid);
          data.toCharArray(str,data.length()+1);
          data = String("/")+String(phone)+String("/house1/state/humid");
          data.toCharArray(pub_topic,data.length()+1);
          client.publish(pub_topic,str);
...
//          client.publish("/buu/iot/diy/state/soil/error","Failed to read from SOIL!!!");
          ...
          data.toCharArray(str,data.length()+1);
          data = String("/")+String(phone)+String("/house1/state/moisture");
          data.toCharArray(pub_topic,data.length()+1);
          client.publish(pub_topic,str);
